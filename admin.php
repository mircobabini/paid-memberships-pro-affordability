 ...
 * the Paid Memberships Pro Affordability plugin
 *
 ...
 *
 * + Search admin.php and {{PLUGIN-SLUG}}.php
 *   for is_multisite() if statements.  Remove the true parts and leave
 *   the false parts.
 * + In {{PLUGIN-SLUG}}.php, go to the initialize() method
 ...
 */
class paid_memberships_pro_affordability_plugin_admin extends paid_memberships_pro_affordability_plugin {
	/**
	...
	public function levels2opts () {
		...
		foreach($levels as $level) {	
			$buf .= "<option value='{$level->id}'>{$level->name}</option>";
		}
